...
- (void)dragTableHeaderDidTriggerRefresh:(DragTableHeaderView_ot*)view
{
    if (self.dragFooterView.isLoading)
    {
        ...
        if ([self.dragDelegateCC respondsToSelector:@selector(dragTableLoadMoreCanceled:)])
        {
            [self.dragDelegateCC dragTableLoadMoreCanceled:self];
        }
    }
...
- (void)dragTableFooterDidTriggerLoadMore:(DragTableFooterView_ot *)view
{
    if (self.dragHeaderView.isLoading)
    {
        ...
        if ([self.dragDelegateCC respondsToSelector:@selector(dragTableRefreshCanceled:)])
        {
            [self.dragDelegateCC dragTableRefreshCanceled:self];
        }
    }
    [self callDelegateDidTriggerLoadMode];
}

- (void)dragTableGesturAdsWillChangeTo:(UIGestureRecognizerState)state observer:(DragTableGestureObserver_ot *)observer
{
...
@dynamic dragDelegateCC;

- (id<UITableViewDragLoadDelegate>)dragDelegateCC
{
...
- (void)callDelegateDidTriggerRefresh
{
    if ([self.dragDelegateCC respondsToSelector:@selector(dragTableDidTriggerRefresh:)])
    {
        [self.dragDelegateCC dragTableDidTriggerRefresh:self];
    }
}

- (void)callDelegateDidTriggerLoadMode
{
    if ([self.dragDelegateCC respondsToSelector:@selector(dragTableDidTriggerLoadMore:)])
    {
        [self.dragDelegateCC dragTableDidTriggerLoadMore:self];
    }
}
